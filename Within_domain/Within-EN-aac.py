...
def EN(X_train, Y_train, X_test, alpha, l1ratio, cv, seed):
    ...
    model = GridSearchCV(EN_pipe, param_grid={"En__alpha": alpha, "En__l1_ratio": l1ratio}, scoring='neg_mean_squared_error', cv=KFold(n_splits=cv, shuffle=True, random_state=seed))
    y_train = Y_train
    x_train = X_train
...
with open('en-res-aac.txt', mode='a') as f:
    f.write('*********************************************\n')
    f.write('AAC\n')
    f.write('*********************************************\n')
    f.close()
    
with open('en-vec-aac.txt', mode='a') as f2:
    f2.write('*********************************************\n')
    f2.write('AAC\n')
    f2.write('*********************************************\n')
    f2.close()    
...
for drug in drugs:
    ...
    for it in range(10):
        ...
        k = 0
        for train_index, test_index in kf.split(CTRP_exprs_drug):
            k+=1
            X_train_N = CTRP_exprs_drug.values[train_index,:]
            y_train = CTRP_aac_drug.values[train_index]

            pred = EN(X_train_N, y_train, CTRP_exprs_drug.values[test_index,:], alph_en, l1, folds, seeds)
            pred_drug.append(pred)
            labels.append(CTRP_aac_drug.values[test_index])
